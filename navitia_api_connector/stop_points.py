...
def _get_stop_points_data():
    stop_points = []

    response = query_navitia("fr-idf", "stop_points", {"depth": 3, "disable_disruption": True})
    next_page = [elem for elem in response["links"] if elem["type"] == "next"]

    for item in response["stop_points"]:
...
def download_stop_points():
    print("     Downloading stop_points data...")
    stop_points = _get_stop_points_data()
    df = pd.DataFrame(stop_points)
    if not os.path.exists("navitia_data"):
        os.mkdir("navitia_data")
    df.to_csv("navitia_data/stop_points.csv")
